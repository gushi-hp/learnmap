(function () {
  ...
  async function aiExpand(word, context) {
    ...
    const systemPrompt = `你是一个学习路径规划专家。给定一个领域关键字，你需要输出学习者在"${ctx}"领域内掌握"${word}"所必须了解的子技能、工具或知识点。你的联想遵循行业学习逻辑——从基础到深入，从理论到实操。你只返回 JSON 数组，不返回其他内容。`;
    const userPrompt = `用户正在学习"${ctx}"领域，现在深入到了"${word}"。请为"${word}"发散 4-8 个学习者需要掌握的下一级知识点。

核心原则：每个词必须和"${word}"直接相关，是学习它必须或值得掌握的具体事物。

发散方向建议（每个方向挑一两个即可，不用全部覆盖）：
- 必备工具/软件：学习"${word}"需要使用什么工具或软件
- 基础理论：需要先理解什么概念
- 实操步骤：需要掌握什么具体技能
- 行业规范/标准：相关的标准或规范
- 进阶方向：深入后可以往哪里走
...
每个词还要附带 tags（标签）字段，规则：
- 2-5 个标签，是字符串数组
- 标签用于在 Obsidian 笔记里做分类和检索，类似 #工具 #理论 #进阶
- 至少包含一个"类型"维度的标签（如 工具、理论、技能、规范、资源、误区、证书、进阶 等）
- 至少包含一个"领域/学科"维度的标签（用"${ctx}"领域里地道的专业术语，越具体越好；进阶或专精节点鼓励使用专业行话作为标签）
- 标签使用中文短词，不要带 # 号、不要带空格（如要分隔用连字符），不要重复
- 不要全是泛词，不同节点的标签应该体现节点本身的差异

要求：
1. 优先产生生动、具体的词，如"CAD快捷键表"优于"CAD知识"
2. 如果当前词已经是最底层、不可再分的具体操作或资源（如软件下载链接、快捷键表、具体命令、练习题），标记 leaf: true
3. 每个词包含 zh、en、leaf、tags 四个字段，严格按 JSON 数组返回，不要其他文字`;
